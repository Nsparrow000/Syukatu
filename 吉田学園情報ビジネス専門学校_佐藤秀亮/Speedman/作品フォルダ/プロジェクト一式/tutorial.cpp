	...
	if (Fade == CFade::FADE_NONE)
	{
			...
			if (bPause == true)
			{
				if (m_bFade == false)
				{
					if (CrossKey == 0.0f)
					{
						...
						if (m_SelectNum <= 0)
						{
							m_SelectNum = MAX_TUTORIALSERECT - 1;
						}
					...
					{
						m_SelectNum++;
						if (m_SelectNum >= MAX_TUTORIALSERECT)
						{
							m_SelectNum = 1;
						}
						CSound::Play(CSound::SOUND_LABEL_SELECT);
		...
		else if (m_pKeyboard != NULL)
		{
			...
			if (bPause == true)
			{
				...
				else if ((m_pKeyboard->GetKey(DIK_RETURN) == true || m_pKeyboard->GetKey(DIK_SPACE) == true) && m_SelectNum == 3)	//戻る
				{
					CFade::SetFade(CManager::MODE_TITLE);
					CSound::Play(CSound::SOUND_LABEL_CLICK);
					CSound::Stop(CSound::SOUND_LABEL_GAMEBGM);
...
//選択番号セット
//*****************************************************************************
void CTutorial::SetSelectNum(int bFlag)
{
	m_SelectNum = bFlag;
}
